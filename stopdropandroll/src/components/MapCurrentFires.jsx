...
import FireSimulation from "@/app/FireSimulation";

const MapCurrentFires = () => {
  ...
  const { handleGetCurrentFires, currentFires } = useGetConditions();
  ...
  const createHeatMap = (evt) => {
    ...
    const geoJsonData = {
      ...
      features: currentFires.map((fire) => ({
        type: "Feature",
        geometry: {
          type: "Point",
          coordinates: [parseFloat(fire[1]), parseFloat(fire[0])],
        },
        properties: {
          dbh: parseFloat(fire[2]) / 10,
        },
      })),
    };
  ...
  const getFires = async () => {
    setLoading(true);
    handleGetCurrentFires(viewState.latitude, viewState.longitude);
  };
...
export default MapCurrentFires;
